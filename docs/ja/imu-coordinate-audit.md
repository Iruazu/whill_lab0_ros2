...
### 8.5.1 B1 数値代替計測 (2026-07-10 追記)

CloudCompare GUI の 3 点ピックが難航したので、`scripts/m5r3_b1_numeric.py`
で数値判定に切り替え。原点半径 5m 円柱の z ヒストグラム (bin 0.1m) から
地面 2 層化を検出する方式:

```
p1 (下): z = -0.793 m (count 731)      ← start 側再訪の地面
p2 (上): z = +0.601 m (count 3,097)    ← end 側再訪の地面 (4 倍多い)
gap    = 1.394 m
```

`traj_lidar.txt` の end-to-start dz = **+1.303 m** と **7.0% 差** (+0.091 m)。
異なる 2 系統の測定 (trajectory の解析 vs. 地図点群の統計) が独立に
同じ物理量を出したことで、SLAM の per-axis 誤差が地図に忠実に反映
されている強い証拠となった。目視 PASS の裏付けとして採用。

壁面 radial 分析では r=4.35 と r=4.65 に近接ピーク (gap 0.30m) が出たが、
この 30cm は物理的な壁厚 (200-300mm) と区別できないため参考値扱い。
角度分離解析までは今回スコープ外。
