...
(nav_launch.py) with an explicit height band `-0.2 .. 1.6 m` to feed
`/scan` for the obstacle_layer. To avoid a dual publisher on `/scan`,
remap the upstream velodyne_laserscan_node's output to `/scan_raw` here.
No downstream consumer exists for `/scan_raw` today; it stays around as
a diagnostic surface if we ever want the pre-filter view.
"""
