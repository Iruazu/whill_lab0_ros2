...
prepare_ros2_layout() {
  # The upstream repo is a dual ROS1/ROS2 tree: it ships package_ROS1.xml,
  # package_ROS2.xml and launch_ROS1/, launch_ROS2/. The expected build
  # entry point is `./build.sh humble`, which (a) wipes the workspace
  # build/ devel/ install/ trees (a non-starter for us — we share the
  # workspace with whill_localization, whill_navigation, glim, etc), and
  # (b) copies package_ROS2.xml -> package.xml and launch_ROS2/ ->
  # launch/ to make the tree look like a normal ament_cmake package.
  # We replicate only step (b) so the rest of the workspace stays
  # untouched and colcon picks the package up. The cleanup that
  # build.sh does at the end (rm -rf launch/) is intentionally skipped
  # because the install/ tree generated by colcon still needs the launch
  # files at runtime.
  local src_xml="${CLONE_DEST}/package_ROS2.xml"
  local dst_xml="${CLONE_DEST}/package.xml"
  if [[ ! -f "${src_xml}" ]]; then
    echo "ERROR: ${src_xml} not found. Upstream layout may have changed." >&2
    exit 1
  fi
  # Always overwrite so a stale package.xml from a previous ROS1 build
  # cannot leak in.
  cp -f "${src_xml}" "${dst_xml}"
  if [[ -d "${CLONE_DEST}/launch_ROS2" ]]; then
    rm -rf "${CLONE_DEST}/launch"
    cp -rf "${CLONE_DEST}/launch_ROS2" "${CLONE_DEST}/launch"
  fi
  echo "Staged package_ROS2.xml -> package.xml and launch_ROS2/ -> launch/."
}
...
verify() {
  ...
  if [[ ! -f "${pkg_xml}" ]]; then
    echo "ERROR: ${pkg_xml} not found after prepare_ros2_layout." >&2
    exit 1
  fi
...
main() {
  ...
  install_gtsam_ppa
  echo "[4/4] clone src/third_party/FAST_LIO_SAM + stage ROS 2 layout"
  clone_or_update
  prepare_ros2_layout
  verify
  print_next_steps
